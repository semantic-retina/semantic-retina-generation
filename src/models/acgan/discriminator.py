...
class Discriminator(nn.Module):
    def __init__(self, in_channels: int, img_size: int, n_classes: int):
        ...
        self.conv_blocks = nn.Sequential(
            *self.discriminator_block(in_channels, hidden_channels),
            *self.discriminator_block(hidden_channels, hidden_channels * 2),
            *self.discriminator_block(hidden_channels * 2, hidden_channels * 4),
            *self.discriminator_block(hidden_channels * 4, hidden_channels * 8),
            *self.discriminator_block(hidden_channels * 8, hidden_channels * 16),
            *self.discriminator_block(hidden_channels * 16, out_channels),
        )
