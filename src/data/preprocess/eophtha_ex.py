...
def process_image(
    ...
    ex_label = np.ones((height, width), dtype="uint8") * GRAY_CLASS["EX"]
    bg_label = np.ones((height, width), dtype="uint8") * GRAY_CLASS["RETINA"]

    ex_img = open_binary_mask(ex_path / change_suffix(str(image_path), f"_EX.png"))
...
def preprocess_eophtha_ex(
    ...
    retina_path = root_path / "e_optha_EX" / "EX"
    ...
    print(f"Preprocessing e-ophtha_EX) with {n_workers} workers...")
